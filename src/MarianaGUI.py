    ...
    async def initPins(self, board) -> None:
        ...
        for m in self.motorNames:
            for p in self.pinNames:
                ...
                task = asyncio.create_task(await board.set_pin_mode_digital_output(pin_number))
                task.set_name(f'set pin {pin_number} as digital output')
                task.add_done_callback(self.check_task_status)
                tasks.append(task)
                '''
                
        for index, p in enumerate(self.joystickPinNames):
            pin_number = int(self.config.get(self.joystickName, p))
            if index==0:
                ...
                task = asyncio.create_task(self.analog_input(board, pin_number))
