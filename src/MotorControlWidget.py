...
class MotorControlWidget(QWidget):
    def __init__(self, parent=None, iLayerout=1):
        ...
        self.vrx_curr = -1;
        self.vrx_prev = -1;
        self.vrx_detection_tolerance = 300
        self.vry_curr = -1;
        self.vry_prev = -1;
        self.vry_detection_tolerance = 300
        self.work_on_xy = True;
        self.sw_pressed_released_count = -1
    ...
    async def move(self, actionId):
        ...
        if not self.parent.motorRunningStatus[motorIndex]==0:
            return
        
        self.parent.motorRunningStatus[motorIndex]=1
    ...
    async def the_callback(self, data):
        ...
        self.parent.motorRunningStatus[motorIndex]=0
    ...
    async def analog_input_x(self, board, pin):
        await board.set_pin_mode_analog_input(pin, self.vrx_detection_tolerance, self.the_vrx_callback)
    async def analog_input_y(self, board, pin):
        await board.set_pin_mode_analog_input(pin, self.vry_detection_tolerance, self.the_vry_callback)
    async def digital_input_pullup(self, board, pin):
        await board.set_pin_mode_digital_input_pullup(pin, self.the_sw_callback)

    @asyncSlot()
    async def setupInputPin(self, actionId):
        board = self.parent.board
        if board is None:
            return
        await self.setupInputPinImpl(board, actionId)
        
    async def setupInputPinImpl(self, board, actionId):    
        tasks = []
        print("starting to setup input Pin")
        for index, p in enumerate(self.parent.joystickPinNames):
            pin_number = int(self.parent.config.get(self.parent.joystickName, p))
            if index==0:
                task = asyncio.create_task(self.digital_input_pullup(board, pin_number))
                task.set_name(f'set pin {pin_number} as digital input')
                tasks.append(task)
            elif index==1:
                task = asyncio.create_task(self.analog_input_x(board, pin_number))
                task.set_name(f'set pin {pin_number} as analog input')
                tasks.append(task)
            else :
                task = asyncio.create_task(self.analog_input_y(board, pin_number))
                task.set_name(f'set pin {pin_number} as analog input')
                tasks.append(task)
        await asyncio.gather(*tasks, return_exceptions=True)
    
        
    async def the_vrx_callback(self, data):
        self.vrx_prev = self.vrx_curr
        self.vrx_curr = data[2]
        if self.vrx_prev>=0:
            print(f'self.vrx_curr={self.vrx_curr}, self.vrx_prev={self.vrx_prev},')
            if self.vrx_curr>400 and self.vrx_curr<600:
                return
            if self.work_on_xy:
                actionId = 0
                if self.vrx_prev > self.vrx_curr:
                    actionId = 10
                self.move(actionId)
            else:
                actionId = 40
    ...
    async def the_vry_callback(self, data):
        self.vry_prev = self.vry_curr
        self.vry_curr = data[2]
        if self.vry_prev>=0:
            print(f'self.vry_curr={self.vry_curr}, self.vry_prev={self.vry_prev},')
            if self.vry_curr>400 and self.vry_curr<600:
                return
            if self.work_on_xy:
                actionId = 20
                if self.vry_prev > self.vry_curr:
                    actionId = 30
                self.move(actionId)
            else:
                actionId = 40
    ...
    async def the_sw_callback(self, data):
        if self.sw_pressed_released_count>=0:
            if self.sw_pressed_released_count%2==1:
                self.work_on_xy = not self.work_on_xy
                print(f'self.work_on_xy={self.work_on_xy}')
        self.sw_pressed_released_count += 1
